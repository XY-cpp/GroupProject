...
template<class T>
T& Vector<T>::operator[](const size_t &t)
{
	assert(this->len > t);
	return this->p[t];
}
...
template<class T>
const T& Vector<T>::operator[](const size_t& t) const
{
	assert(this->len > t);
	return this->p[t];
}
...
template<class T>
bool Vector<T>::operator==(const Vector<T>& t)
{
...
template<class T>
void Vector<T>::resize(size_t lenNew)
{
	T *pNew = new T[lenNew];
